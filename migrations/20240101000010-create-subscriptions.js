...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    await queryInterface.createTable('subscription', {
      ...
      purchase_date: { type: Sequelize.DATE, allowNull: false, defaultValue: Sequelize.NOW },
      expiry_date: { type: Sequelize.DATE, allowNull: true },
      created_at: { type: Sequelize.DATE, allowNull: false, defaultValue: Sequelize.NOW },
      updated_at: { type: Sequelize.DATE, allowNull: false, defaultValue: Sequelize.NOW }
    });
    await queryInterface.addIndex('subscription', ['status']).catch(() => {});
    await queryInterface.addIndex('subscription', ['transaction_id']).catch(() => {});
  },
  down: async (queryInterface) => { await queryInterface.dropTable('subscription'); }
};
